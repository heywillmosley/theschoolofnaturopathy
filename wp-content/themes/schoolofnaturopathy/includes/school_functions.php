...
// Render Program Landing Page
function render_program_landing ($display = TRUE) {
	...
	if( EWSN_PROGRAMS ) {
		...
		foreach( EWSN_PROGRAMS as $program ) {
			if($program['program_landing'] == get_permalink() ) {
				...
				$render .= render_catalog_download_btn($program['download_catalog']);
				$render .= wp_get_attachment_image( $program['program_feature_image']['ID'], $image_size );
				$render .= testimony($program['testimonials'][0]);
				$render .= "</div>"; // end col
				$render .= "</div>"; // end row
				$render .= render_program_overview($program['program_overview']);
				$render .= render_what_youll_learn($program['what_youll_learn']);
				$render .= render_track_length($program['track_length']);
				$render .= render_track_hours($program['number_of_hours_for_program']);
				$render .= render_problems( $program['problem_solved'] );
				$render .= testimony($program['testimonials'][1]);
				$render .= render_features( $program['feature'] );
				$render .= $program['closing_reason_to_enroll'];
				$render .= "</hr>";
				$render .= render_app_btn($program['application_link']);
				$render .= render_faq( $program['faq'] );
				$render .= render_ewsn_debug($program);
		...
		if ($display) 
			...
			return $render; // get or set render

	} // end if
} // end render_program_landing
