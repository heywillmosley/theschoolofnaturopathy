...
if ( 0 < $total_lessons ) {
    ...
    $html .= '<section class="course-lessons">';
    	...
    	$html .= '<header>';
    	  $html .= '<h3>' . __( 'Course Lessons:', 'woothemes-sensei' ) . '</h3>';
    	...
    	foreach ($course_lessons as $lesson_item){
    	    ...
    	    $html .= '<article class="' . esc_attr( join( ' ', get_post_class( array( 'media course', 'post' ), $lesson_item->ID ) ) ) . '">';
            $html .= '<div class"pull-left media-object">';
                // Image
    			...
    			$html .= '<header>';
    	    		...
    	    		$html .= '<h3><a href="' . esc_url( get_permalink( $lesson_item->ID ) ) . '" title="' . esc_attr( sprintf( __( 'Start %s', 'woothemes-sensei' ), $lesson_item->post_title ) ) . '">';

                    if( apply_filters( 'sensei_show_lesson_numbers', $show_lesson_numbers ) ) {
                        $html .= '<span class="lesson-number">' . $lesson_count . '. </span>';
                    }

                    $html .= esc_html( sprintf( __( '%s', 'woothemes-sensei' ), $lesson_item->post_title ) ) . '</a></h3>';
