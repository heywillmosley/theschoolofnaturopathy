<?php
/**
 * Plugin Name: EWSN Programs
 * Plugin URI: theschoolofnaturopathy.com
 * Description: Display's EWSN school programs in line with Learndash and Advance Custom Fields.
 * Version: 1.0
 * Author: Will Mosley
...
class EWSNPrograms 
{
	...
	function __construct() {
		...
		add_action( 'init', array( $this, 'setPrograms' ) );
		add_action( 'init', array( $this, 'setCurrentUserMembershipIDs' ) );

		if ( basename( get_permalink() ) == 'programs' ) {
        add_filter('page_template', array( $this, 'getPageSchoolPrograms' ) );
       	}
	...
	public function enqueue_scripts_and_css() {
        wp_register_style( 'ewsn-programs', plugin_dir_url( __FILE__ ) . 'css/school.css' );
        wp_enqueue_style( 'ewsn-programs' );
    }
	...
	function getPageSchoolPrograms() {
		echo $this->setPageSchoolPrograms();
	}
	...
	function setCurrentUserMembershipIDs() { // return array
		...
		add_action('plugins_loaded', 'WC_Memberships');
...
$ewsn_programs = new EWSNPrograms;